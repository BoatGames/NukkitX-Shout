...
public class WindowsFrom implements Listener {
    ...
    public void displayWindow(Player player, ShoutConfig config){
        FormWindowCustom custom = new FormWindowCustom(config.msg.title);
        custom.addElement(new ElementLabel(config.msg.subTitle));
        custom.addElement(new ElementInput(config.msg.input));
        custom.addElement(new ElementDropdown(config.msg.chose.title,config.msg.chose.list));
        custom.addElement(new ElementDropdown(config.msg.chose1.title,config.msg.chose1.list));
